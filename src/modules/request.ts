...
async function connectDB(client: Client) {
  try {
    ...
    console.error('Error connecting to the database', err);
    throw err;  // Re-throw the error so that it can be caught in tests
  }
}

// Method to get all employee working status regardless of approval
export async function fetchRequestsByDate(date: string) {
    ...
    try {
        ...
        console.error('Error querying the database', err);
        throw err; // Propagate the error so tests can catch it
    } finally {
        await client.end();
    }
}

// Method to get employees who have approved WFH on a certain date
export async function fetchRequestsByDateApproved(date: string) {
...
// Method to get WFH details by type, status, and date
export async function fetchRequestsByWFHDetails(type: string, status: string, date: string) {
    // Validate date format
    if (!isValidDate(date)) {
        throw new Error('Invalid date format');
    }
    ...
    try {
        ...
        const res = await client.query(query, [type, status, date]);
        console.log(res.rows)
        return res.rows;
    } catch (err) {
        console.error('Error querying the database', err);
...
// Method to fetch employees working from the office on a given date
export async function fetchEmployeesWFOByDate(date: string) {
    // Validate date format
    if (!isValidDate(date)) {
        throw new Error('Invalid date format');
    }
...
// Method to count employees working from the office on a given date
// export async function countEmployeesWFOByDate(date: string) {
//     // Validate date format
//     if (!isValidDate(date)) {
//         throw new Error('Invalid date format');
//     }
...
//     }
// }
// Method to fetch requests by department and date
// Method to get employees who have approved WFH based on DEPT on the certain date
export async function fetchRequestsByDeptDate(dept: string, date: string, status: string) {
  // Validate date format before proceeding
  if (!isValidDate(date)) {
  ...
  try {
      ...
      console.error('Error querying the database', err);
      throw err;  // Ensure the error is thrown for proper error handling
  } finally {
      await client.end();
  }
}
